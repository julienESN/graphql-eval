...
const IndexScreen: React.FC = (): ReactElement => {
  ...
  const [selectedAuthors, setSelectedAuthors] = useState<Set<number>>(
    new Set()
  );
  const [sortOrder, setSortOrder] = useState<'ASC' | 'DESC' | null>(null);
  const [sortByLikes, setSortByLikes] = useState<
    'MOST_LIKED' | 'LEAST_LIKED' | null
  >(null);
  ...
  const filteredData = data
    .filter((article) => {
      ...
      const matchesAuthor =
        selectedAuthors.size === 0 || selectedAuthors.has(article.author.id);
  ...
  const handleSortChange = (
    value: 'ASC' | 'DESC' | 'MOST_LIKED' | 'LEAST_LIKED'
  ) => {
    if (value === 'MOST_LIKED') {
  ...
  return (
    <div className="p-6 mx-32">
      ...
      <div className="mb-4 flex flex-row items-center space-x-4">
        ...
        <div className="flex items-center">
          <MultiSelect
            options={data.reduce((uniqueAuthors, article) => {
              if (
                !uniqueAuthors.some(
                  (author) => author.value === article.author.id.toString()
                )
              ) {
                uniqueAuthors.push({
                  label: article.author.name,
                  value: article.author.id.toString(),
                });
              }
            ...
            onValueChange={(selectedOptions) => {
              const selectedIds = new Set(
                selectedOptions.map((option) => Number(option))
              );
              setSelectedAuthors(selectedIds);
            }}
            defaultValue={[...selectedAuthors].map((authorId) =>
              authorId.toString()
            )}
        ...
        <div className="flex items-center">
          ...
          <Select
            ...
            onValueChange={(value) =>
              handleSortChange(
                value as 'ASC' | 'DESC' | 'MOST_LIKED' | 'LEAST_LIKED'
              )
            }
          >
      ...
      {filteredData.length === 0 && !isLoading && (
        <p>Aucun article trouvé pour les auteurs sélectionnés.</p>
      )}
        ...
        {filteredData.map((item) => (
          <Article
            ...
            isLiked={item.likes.some(
              (like) => like.user.id === userData?.me?.id
            )}
          />
        ))}
      </div>
